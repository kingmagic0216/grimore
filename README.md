...
A working grimoire of the Western esoteric tradition — twenty-nine chapters, 592 pages, 75 diagrams and 7 plates — in which **every source is dated and attributed, and the attribution is allowed to be inconvenient.**
...
| **PDF** | [`grimoire.pdf`](grimoire.pdf) — 22 MB | The typeset book: 6×9 in Cardo, cut after the Bembo of 1495; indented paragraphs, raised initials, small-capital heads, running heads and folios. |
...
- **The seventy-two spirits of the *Goetia* are given entire** — each with its rank, the shape it is said to arrive in, what it is called for, and the legions under it. Read down the offices and the roster stops being a bestiary and becomes an index of wants: languages, a lost thing found, the thief brought back with the goods, what another person is thinking. Nine of them are given a former rank in the celestial orders, and three are serving a sentence and expect to go back.
...
The checks on their own, against whatever is currently built. Internal links resolve to real anchors, no id is used twice, no chapter repeats or skips a figure number, no two labels on one line of a figure run into each other, no SVG `<text>` contains markup that will not render, the EPUB is a well-formed archive whose XML parses, the source file is CRLF throughout because the cross-reference signatures are cut at a fixed character distance and a stray conversion to LF shifts every one of them at once, every chapter reference — linked, prose, or a bare numeral in a table cell — names the chapter it actually points at, the Materials chapter's every derived count is re-derived from the rite table it summarises — **and every page number in the index is a page the term is actually on.**
